...
struct ContentView: View {
    @EnvironmentObject var store: PatchBookStore
    ...
    @ViewBuilder
    private func row(for entry: PatchEntry) -> some View {
        HStack {
...
struct EntryEditorView: View {
    ...
    let existing: PatchEntry?
    let onSave: (PatchEntry) -> Void

    init(entry: PatchEntry?, onSave: @escaping (PatchEntry) -> Void) {
        self.existing = entry
        self.onSave = onSave
        ...
        NavigationStack {
            Form {
                Section("Patch") {
                    ...
                    TextField("Event / Source", text: $detail)
            ...
            .navigationTitle(existing == nil ? "Add Patch" : "Edit Patch")
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Save") {
                        var entry = existing ?? PatchEntry(name: name, detail: detail, date: date)
                        entry.name = name
                        entry.detail = detail
                        entry.date = date
